...
describe('pickStageNext', () => {
  const base = { hasDrain: false, queuedCount: 0, oldestQueuedAtMs: null, nowMs: 10_000 };
  ...
  it('飢餓弁: 最古が BANNER_STARVE_MS 待ったらバナーが割り込む(境界含む)', () => {
    const now = 100_000;
    const just = { hasDrain: true, queuedCount: 1, oldestQueuedAtMs: now - BANNER_STARVE_MS, nowMs: now };
